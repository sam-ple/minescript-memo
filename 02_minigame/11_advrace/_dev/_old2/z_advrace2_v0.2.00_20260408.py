...
def save_state():
    ...
    data = {
        ...
        "pickup_targets": list(pickup_targets),
...
def load_state():
    ...
    pickup_targets = set(data.get("pickup_targets", []))
...
def calc_points(adv, adv_type):
    ...
    # =========================
    if CATEGORY_MODE:
        if adv_type == "reached the goal":
            base = 2
        elif adv_type == "completed the challenge":
            base = 3

    # =========================
    # PICKUP BONUS（+1方式）
    # =========================
    if PICKUP_MODE and adv in pickup_targets:
        base += 1
        chat(f"🎯 PICKUP BONUS! {adv} (+1)", "green")

    # =========================
    # FIRST BONUS
    # =========================
    if FIRST_MODE and adv not in first_advancements:
        first_advancements.add(adv)
        base += 1
        is_first = True
        chat(f"✨ FIRST! {adv} (+1)", "gold")

    return base, is_first
...
def start_game():
    ...
    # =========================
    if PICKUP_MODE:
        # pickup_targets = set(random.sample(GOAL_ADVANCEMENTS, 3))
        pickup_targets = set(random.sample(PICKUP_POOL, 3))

        chat("🎯 PICKUP MISSIONS!", "gold")
        for adv in pickup_targets:
            chat(f" - {adv}", "yellow")

    # モード表示（任意だけど便利）
    if PICKUP_MODE:
        chat("Mode: PICKUP", "gold")
    elif CATEGORY_MODE:
        chat("Mode: CATEGORY", "yellow")
    else:
        chat("Mode: NORMAL", "white")
...
def restart_game():
    ...
    global PICKUP_MODE, CATEGORY_MODE, FIRST_MODE
    ...
    PICKUP_MODE = data.get("pickup_mode", False)
    CATEGORY_MODE = data.get("category_mode", False)
    FIRST_MODE = data.get("first_mode", True)
...
while True:
    ...
    if event and event.type == EventType.CHAT and game_active:
        ...
        if m_:
            ...
            player, adv_type, adv = m_.groups()  # ★変更

            pt, is_first = calc_points(adv, adv_type)  # ★変更
